...
)
{
    ...
    static std::string cached_path = "";        // 마지막으로 검사한 이미지 경로
    static cv::Mat cached_src;                  // 마지막으로 로드한 원본 사진
    static cv::Mat cached_raw_heatmap;          // 마지막으로 뽑아낸 히트맵
    ...
    else
    {
        ...
        if (path != cached_path || cached_raw_heatmap.empty() || cached_src.empty())
        {
            ...
            // 4. [1, 1, 256, 256] 텐서를 256x256 2차원 히트맵 이미지(Mat)로 변환
            cv::Mat temp_heatmap(256, 256, CV_32F, outputs[0].ptr<float>());
